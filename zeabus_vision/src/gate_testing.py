...
def is_pipe(cnt):
    ...
    if (not (area_ratio > area_ratio_expected
             and wh_ratio > wh_ratio_expected
...
def find_pipe(binary, number_of_object, align):
    ...
    if align == 'v':
        ...
        cv.imshow('pipe_vertical', pipe)
    else:
        result = sorted(result, key=itemgetter(2), reverse=True)
        cv.imshow('pipe_horizontal', pipe)
...
def find_gate():
    ...
    for res in horizontal_pipe + vertical_pipe:
        ...
        cv.rectangle(display, (int(x - w / 2.), int(y - h / 2.)),
                     (int(x + w / 2.), int(y + h / 2.)), (255, 0, 255), 2)
    ...
    if no_pipe_v + no_pipe_h >= 2:
        ...
        for res in horizontal_pipe + vertical_pipe:
            x, y, w, h, angle = res
            cv.rectangle(gate, (int(x - w / 2.), int(y - h / 2.)),

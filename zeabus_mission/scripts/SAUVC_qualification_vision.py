		...
		while( not rospy.is_shutdown() ):
			...
			if( self.vision.num_object() == 1 ):
				...
				if( count_not_found == 5 ):
					self.run_type = 0
					self.relative_xy( 10 , math.copysign( 1.5 , -1 * self.type_pier ) )
	...
	def type_3( self ):	
		...
		while( not rospy.is_shutdown() ):
			...
			self.echo( self.name , "Type 3 and move slide to don't have object : " + 
					str(count_not_found) )
			self.velocity( {'y' : math.copysign( 0.15 , -1 * self.type_pier )} )	
			if( self.vision.have_object() ):
				count_not_found = 0
			else:
				count_not_found += 1
		...
		while( not rospy.is_shutdown() ):
			...
			self.velocity( {'y' : math.copysign( 0.10 , -1 * self.type_pier )} )	
			self.echo( self.name , "Type3 now move time is " + str( diff_time ) )
			if( diff_time > 3 ): break
